window.addEventListener("message", async e => {
  ...
  let video_id = video_config_media['metadata'];
  ...
  (() => {
    ...
    let forward_tooltipText = "Adelantar 30s";
    ...
    jwplayer().on('ready', () => {
      ...
      displayError(`Más información en la Consola.\n${linkIssue(`Código: ${e.code}`)}`)
      console.error(e)
    })
    ...
    // Guarda el tiempo del video cada 7 segundos.
    setInterval(() => {
      if (jwplayer().getState() == "playing")
  ...
  /* ~~~~~~~~~~ FUNCIONES ~~~~~~~~~~ */
  // MP4 (download) - Premium: Obtenga el enlace directo del tráiler
  function getDirectFile(url) {
    return url.replace(/\/clipFrom.*?index.m3u8/, '').replace('_,', '_').replace(url.split("/")[2], "fy.v.vrv.co");
  }

  // MP4 (download) - Gratis: Obtenga el enlace directo por defecto
  ...
  // M3U8 (assistir) - Premium: Obtenga el enlace directo del tráiler
  function getDirectStream(url, idx) {
    // TODO: assistir premium pelo m3u8 (por enquanto usamos o mp4)
  }

  // M3U8 (assistir) - Gratis: Obtenga el enlace directo por defecto
  ...
  function getSourceLocale() {
    try {
      ...
      displayError(`Las cookies ${linkIssue("se desactivan")}!</code>`);
      throw err;
    }
  }

  function linkIssue(text, issue = "") {
    return `<a href="https://github.com/VirtualOx-sys/crunchyroll-premium/issues/${issue}" target="_blank" style="color: rgb(244, 117, 33)">${text}</a>`
  }

  function displayError(info) {
    const msg = "¡Error al cargar el video! (>﹏<)\n" + info;
